...
def generator_note(previous: dict, current: dict) -> str:
    """Say when a drift is more likely a compiler difference than an API one.

    The snapshot holds clang's JSON AST verbatim, and that representation
    changes between clang releases without any declaration changing. A report
    generated by a different clang from the one that wrote the snapshot is
    therefore expected to differ, and a message that only said "the API
    changed" would send the reader to look for a change that is not there.

    A snapshot written before this field existed records nothing, and the note
    says that rather than guessing.
    """
    recorded = previous.get("generator")
    running = current.get("generator", "unknown")
    if recorded is None:
        return (
            "\n  note: this snapshot records no generating compiler, so a "
            "difference here may be clang's AST representation rather than the "
            f"API. This run used: {running}"
        )
    if recorded != running:
        return (
            "\n  note: the snapshot was written by a different compiler, so "
            "part of this difference is likely its AST representation rather "
            f"than the API.\n    snapshot: {recorded}\n    this run: {running}"
        )
    return ""


def require_breaking_api_acceptance(
    report: dict, acceptance: Path | None, note: str = ""
) -> None:
    breaking = any(
        report.get(key, [])
        for key in (
...
def collect_snapshot(
    ...
    # METADATA, AND DELIBERATELY OUTSIDE `api`. `compare_reports` diffs `api`
    # wholesale and `classify_api_drift` reads it as declarations, so a
    # compiler identity living in there would BE an API difference on every
    # machine but the one that wrote the snapshot -- the opposite of what it
    # is for. Top level, and absent from the compared key list below.
    api_snapshot = dict(api_snapshot)
    generator = api_snapshot.pop('generator', 'unknown')
...
def main() -> int:
    ...
    if args.output:
        ...
        if output.is_file():
            try:
                ...
                require_breaking_api_acceptance(
                    ...
                    generator_note(previous, report))
        ...
        if expected == report:
            ...
            note = generator_note(expected, report)
            try:
                require_breaking_api_acceptance(
                    drift, args.accept_breaking_api, note)
